...
async function seedExperiencesCollection() {
  ...
  try {
    ...
    console.log(`Found ${posts.length} posts to transform into experiences`);
    
    if (posts.length === 0) {
      console.log('No posts found to seed the experiences collection.');
      return;
    }
    ...
    const experiences = posts.map(post => {
      ...
      return {
        ...
        image: imageUrl,
        title: post.title || 'Experience in Shanghai',
